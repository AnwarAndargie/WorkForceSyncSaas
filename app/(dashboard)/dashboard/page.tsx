...
export default function DashboardPage() {
  ...
  const {
    ...
    user?.role === "super_admin" ? "/api/organizations" : null,
    fetcher
  );
  const {
    data: plans,
    ...
    user?.role === "super_admin" ? "/api/plans" : null,
    fetcher
  );
  ...
  const {
    ...
    user?.role === "org_admin" && user?.organizationId
      ? `/api/organization/${user.organizationId}/users`
      : null,
    fetcher
  );
  ...
  const {
    data: orgPlanData,
    error: orgPlanDataError,
    isLoading: orgPlanDataLoading,
  } = useSWR<Plan>(
    user?.role === "org_admin" ? `/api/plans/${orgData?.planId}` : null,
    fetcher
  );
  ...
  const orgAnalytics: OrganizationAnalytics | null = organizations
    ? {
        ...
        freeTrial: organizations.filter(
          (org) => org.stripeSubscriptionStatus === "trialing"
        ).length,
        isPaid: organizations.filter(
          (org) => org.stripeSubscriptionStatus === "active"
        ).length,
      }
    : null;
  ...
  const orgAdminAnalytics: OrgAdminAnalytics | null = orgUsers
    ? {
        ...
        conversionRate:
          orgUsers.length > 0
            ? Math.round(
                (orgUsers.filter((user) => user.isActive).length /
                  orgUsers.length) *
                  100
              )
            : 0,
        activeMembersCount: orgUsers.filter(
        ...
        totalEarnings: orgData?.planId
          ? (plans?.find((p) => p.id === orgData.planId)?.price || 0) * 12
          : 0,
      }
    : null;
        ...
        <>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            <Card className="p-4">
              ...
              <CardContent className="p-0">
                <p className="text-2xl font-bold">
                  {orgAdminAnalytics.totalUsers}
                </p>
                <p className="text-sm text-gray-500">
                  {orgAdminAnalytics.newUsersThisMonth > 0 &&
                    `+${orgAdminAnalytics.newUsersThisMonth} this month`}
                </p>
              </CardContent>
            </Card>
            ...
            <Card className="p-4">
              ...
              <CardContent className="p-0">
                <p className="text-2xl font-bold">
                  {orgAdminAnalytics.conversionRate}%
                </p>
                <p className="text-sm text-gray-500">
                  {orgAdminAnalytics.activeUsers} active users
                </p>
              </CardContent>
            </Card>
            ...
            <Card className="p-4">
              ...
              <CardContent className="p-0">
                <p className="text-2xl font-bold">
                  {orgAdminAnalytics.activeMembersCount}
                </p>
            ...
            <Card className="p-4">
              ...
              <CardContent className="p-0">
                <p className="text-2xl font-bold">
                  ${orgAdminAnalytics.totalEarnings.toFixed(2)}
                </p>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
            ...
            <Card className="p-4">
              ...
              <CardContent className="space-y-2">
                <p>
                  <strong>Plan:</strong> {orgPlanData?.name || "No Plan"}
                </p>
                <p>
                  <strong>Status:</strong>{" "}
                  {orgPlanData?.isActive ? "active" : "Inactive"}
                </p>
                <p>
                  <strong>Next Billing Date:</strong>{" "}
                  {Date() ? new Date().toLocaleDateString() : "Not Available"}
                </p>
                <p>
                  <strong>Amount:</strong> ${orgPlanData?.price.toFixed(2)}
                  /month
                </p>

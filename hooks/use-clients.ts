...
export function useClients(page = 1, limit = 10, search = "") {
  ...
  const updateClient = useCallback(async (id: string, clientData: UpdateClientData) => {
    setIsUpdating(true);
    try {
      ...
      if (!response.ok) {
        const errorData = await response.json();
        throw new Error(errorData.error?.message || "Failed to update client");
      }
  ...
  const deleteClient = useCallback(async (id: string) => {
    ...
    try {
      ...
      if (!response.ok) {
        const errorData = await response.json();
        throw new Error(errorData.error?.message || "Failed to delete client");
      }
